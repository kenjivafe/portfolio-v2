import { Antigravity, Claude, OpenAI, Windsurf } from '@lobehub/icons';
...
const TECH_STACK = [
  ...
  // Row 4: Infrastructure & Mobile
  { name: 'Git', slug: 'git' },
  { name: 'Vercel', slug: 'vercel' },
  { name: 'Railway', slug: 'railway' },
  { name: 'Expo', slug: 'expo' },
  
  // Row 5: Creative Suite
...
const TechIcon = ({ slug, name }: { slug: string; name: string }) => {
  const isDarkIcon = ['nextdotjs', 'vercel', 'shadcnui', 'expo', 'railway', 'prisma', 'express'].includes(slug);
  const iconClass = `${styles['tech-icon']} ${isDarkIcon ? styles['is-dark-icon'] : ''}`;
  const svgClass = `${styles['tech-icon-svg']} ${isDarkIcon ? styles['is-dark-icon'] : ''}`;

  // 1. LobeHub Icons (AI / Brand)
  if (slug === 'antigravity') {
    const Icon = (Antigravity as any).Color || Antigravity;
    return <Icon size={20} className={svgClass} />;
  }
  if (slug === 'claude') {
    const Icon = (Claude as any).Color || Claude;
    return <Icon size={20} className={svgClass} />;
  }
  if (slug === 'openai' || slug === 'chatgpt') {
    const Icon = (OpenAI as any).Color || OpenAI;
    return <Icon size={20} className={svgClass} />;
  }
  if (slug === 'codeium' || slug === 'windsurf') {
    const Icon = (Windsurf as any).Color || Windsurf;
    return <Icon size={20} className={svgClass} />;
  }

  // 2. Skill Icons (PS and AI)
  if (slug === 'ps' || slug === 'ai') {
    return (
      <img 
  ...
  // 3. Canva Special (Custom SVG provided by user)
  if (slug === 'canva') {
    return (
      <svg className={svgClass} viewBox="0 0 508 508" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd" strokeLinejoin="round" strokeMiterlimit="2">
  ...
  // 4. Default Simple Icons (Raw SVG paths from CDN)
  return (
    <img 
...
export default function Skills() {
  return (
    <section className={styles['skills-section']} id="capabilities">
        ...
        <div className={styles['capabilities-grid']}>
          {TECH_STACK.map((tech, i) => (
            <Reveal 
              key={tech.name} 
              delay={`d${i % 4}` as any} 
              className={styles['tech-cell']}
            >
              <div className={styles['tech-inner']}>
